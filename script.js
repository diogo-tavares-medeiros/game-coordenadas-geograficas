...
function jogada() {
  ...
  for (let v = 0; v <= 4; v++) {
    if (latJogada == ships[v].lat && lonJogada == ships[v].long) {
      ...
      abrirModal(
        latitudeJogada,
        direcaoLatitudeJogada,
        longitudeJogada,
        direcaoLongitudeJogada,
        'verde'
      )
...
function desenharNavio(posicaoY, posicaoX, cor) {
  ...
  navio.setAttribute('src', `./image/navio-${cor}.png`)
...
function gameOver(pontosGreen, pontosOrange) {
  if (pontosGreen >= 5) {
    aviso.textContent =
      'Equipe Laranja encontrou os 5 navios verdes de pesca ilegal'
    location.href = '#abrirModal2'
    atualizarPagina()
  }
  if (pontosOrange >= 5) {
    aviso.textContent =
      'Equipe Verde encontrou os 5 navios laranja de pirataria'
    location.href = '#abrirModal2'
    atualizarPagina()
  }
}
